...
export const generatePaymentUrl = async (req, res) => {
  ...
  // Use frontend URL for return URL to avoid SSL issues with localhost
...
export const handlePaymentResponse = async (req, res) => {
  ...
  try {
    ...
    if (vnp_ResponseCode !== "00") {
      ...
      redirectUrl = `${frontendUrl}/failed`;
    } else {
      order.order_status = "completed";
      status = "completed";
      redirectUrl = `${frontendUrl}/success`;
...
// New endpoint to handle payment callback from frontend
export const handlePaymentCallback = async (req, res) => {
  ...
  try {
    ...
    console.error("VNPAY handlePaymentCallback error:", error);
    if (req.body && req.body.vnp_TxnRef) {
      try {
