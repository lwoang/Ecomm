...
const ChatWidget = () => {
  ...
  const getWebSocketUrl = () => {
    ...
    if (!url) {
      ...
      // Otherwise, add ws:// for local development
      url = `ws://${url}`;
    }
  ...
  const setupWebSocketHandlers = useCallback((wsInstance) => {
    wsInstance.onopen = () => {
      console.log("WebSocket connected");
      wsInstance.send(JSON.stringify({ type: "init", userId }));
    };
    ...
    wsInstance.onclose = (event) => {
      ...
      if (event.code !== 1000) {
        // Abnormal closure, try to reconnect after a delay
        setTimeout(() => {
          if (ws.current === wsInstance) {
            console.log("Attempting to reconnect WebSocket...");
            const newWs = new WebSocket(`${getWebSocketUrl()}?userId=${userId}`);
    ...
    wsInstance.onerror = (err) => {
      ...
      setMessages((prev) => [
        ...prev,
        { sender: "ai", text: "Connection error. Please check your internet connection." },
      ]);
      setIsLoading(false);
    };
  }, [userId, setMessages, setIsLoading]);
  ...
  useEffect(() => {
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { sender: "ai", text: "Failed to establish connection. Please try again later." },
      ]);
    }
  ...
  const clearChat = () => {
    ...
    if (ws.current && ws.current.readyState === WebSocket.OPEN) {
      ...
      ws.current.close(1000, "Clearing chat session");
    }

    // The new WebSocket will be created automatically when userId changes
