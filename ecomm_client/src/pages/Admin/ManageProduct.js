...
import { UploadOutlined, HomeOutlined, AppstoreOutlined } from "@ant-design/icons";
...
const ManageProduct = () => {
  ...
  if (currentView === "list") {
    return (
      <div>
        {renderBreadcrumb()}
        <ProductList onCreateNew={handleCreateNew} onEdit={handleEdit} />
      </div>
    );
  }
...
const ProductForm = ({ editingProduct, onBack, onSuccess, isEdit }) => {
  ...
  useEffect(() => {
    if (isEdit && editingProduct) {
      ...
      setSelectedCategory(editingProduct.category_id);
      
      if (editingProduct.variations && editingProduct.variations.length > 0) {
        setVariations(editingProduct.variations.map(v => ({
          size: v.size || "",
          color: v.color || "",
          price: v.price || "",
      ...
      // Note: Images from existing product would need special handling
      // as they're already uploaded to the server
    }
  ...
  useEffect(() => {
    ...
    if (Array.isArray(categories) && categories.length > 0 && product.category_id) {
  ...
  const handleSubmit = async () => {
    ...
    confirm({
      ...
      content: `Are you sure you want to ${isEdit ? "update" : "add"} this product?`,
      okText: "Yes",
      okType: "primary",
      cancelText: "No",
      onOk: async () => {
          ...
          >
            ...
            <Form.Item label="Category" className="mb-6">
              <Select
                ...
                className="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500"
                options={Array.isArray(categories) && categories.length > 0 ? categories.map((category) => ({
                  label: category.name,
                  value: category._id,
                })) : []}
                required
